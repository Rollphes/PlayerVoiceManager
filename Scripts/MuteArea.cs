...
namespace io.github.rollphes.playerVoiceManager
{
    ...
    public class MuteArea : BaseVoiceSetting
    {
        ...
        public override int[] LocalPlayerIds {
            get {
                var playerIdList = new DataList();
                if (this._inPlayerIdList.Contains(Networking.LocalPlayer.playerId)) {
                    playerIdList = this._outPlayerIdList;
                    for (var i = 0; i < this._inPlayerIdList.Count; i++) {
                        playerIdList.Add(this._inPlayerIdList[i].Int);
                    }
                } else {
                    playerIdList = this._inPlayerIdList;
                }
